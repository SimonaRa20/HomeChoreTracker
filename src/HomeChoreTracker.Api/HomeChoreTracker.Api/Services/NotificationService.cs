	...
	public class NotificationService : BackgroundService
	{
		...
		private async Task UpdateCurrentEvents()
		{
			using (var scope = _serviceProvider.CreateScope())
			{
				...
				List<Challenge> wasUpdated = new List<Challenge>();
				...
				if(wasUpdated.Count > 0)
				{
                    foreach (var challenge in wasUpdated)
                    {
                        ...
                        else if (challenge.OpponentType.Equals(OpponentType.Home) && challenge.ResultType != ChallengeResultType.None)
                        {
